...
async function detectPowerShell() {
  if (await commandExists('pwsh')) {
    const version = await captureVersion('pwsh', ['-NoLogo', '-NoProfile', '-Command', '$PSVersionTable.PSVersion.ToString()']);
    const majorVersion = parseMajorVersion(version);
    const ok = version !== 'not found' && majorVersion !== undefined && majorVersion >= 7;
    ...
    return {
      ...
      ok,
      version,
      notes: ok ? 'PowerShell 7+ executable detected.' : 'PowerShell was found, but version 7+ is required for the PoC.',
    };
  }

  if (await commandExists('powershell')) {
    const version = await captureVersion('powershell', ['-NoLogo', '-NoProfile', '-Command', '$PSVersionTable.PSVersion.ToString()']);
    ...
    return {
      ...
      ok: false,
      version,
      notes: 'Windows PowerShell was detected, but PowerShell 7+ (pwsh) is required for the PoC.',
    };
  }
...
export function resolveCommandCheck({ name, command, required = true, exists, version, notes }) {
  const runnable = exists && version !== 'not found';

  return {
    name,
    command,
    required,
    ok: required ? runnable : true,
    version,
    notes: runnable ? notes.available : notes.missing,
  };
}

async function detectCommand({ name, command, args = ['--version'], required = true, notes }) {
  const exists = await commandExists(command);
  const version = exists ? await captureVersion(command, args) : 'not found';

  return resolveCommandCheck({ name, command, required, exists, version, notes });
}
